...
describe('getLastWednesdayOfMonth', () => {
  ...
  test('returns normal response when last day of month is not wednesday', () => {
    ...
    jest.spyOn(moment, 'now').mockImplementation(() => mockDate);
    const expectedLastWednesday = moment('2024-01-31')
      .format('DDMMMYYYY')
      .toUpperCase();
    const lastWednesday: Moment | null =
      getLastWednesdayOfMonth() as Moment | null;
    if (lastWednesday !== null)
      expect(lastWednesday.format('DDMMMYYYY').toUpperCase()).toBe(
        expectedLastWednesday,
      );
